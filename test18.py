...
listening_sock.bind(('localhost', 52120+args.edge_id))
...
if args.edge_id==1:
    ...
    sock_node_3.connect(('localhost', 52120))
...
if args.edge_id==2:
    ...
    sock_node_4.connect(('localhost', 52120))
    
    sock_node_1=socket.socket()
    sock_node_1.connect(('localhost', 52121))
...
if args.edge_id==3:
    ...
    sock_node_5.connect(('localhost', 52120))
    sock_node_2=socket.socket()
    sock_node_2.connect(('localhost', 52121))
...
if args.edge_id==4:
    ...
    sock_node_6.connect(('localhost', 52120))
    sock_node_7=socket.socket()
    sock_node_7.connect(('localhost', 52122))
    sock_node_8=socket.socket()
    sock_node_8.connect(('localhost', 52123))

if args.edge_id==5:
    sock_node_9=socket.socket()
    sock_node_9.connect(('localhost', 52120))
    sock_node_10=socket.socket()
    sock_node_10.connect(('localhost', 52122))
...
if args.edge_id!=0:
    ...
    trainset = datasets.ImageFolder('/data/pcqu/Dataset/Dataset/cifar10/device_5_noniid_rate_05/device'+str(edge_num)+'/', transform=transform)
...
'''
    ...
    if epoch<200:
        if args.edge_id==1:
            ...
            send_model=scale_model(send_model,-1)
            final_model=add_model(send_model,rec_model)
            ''' n=0
            count=0
            for model in final_model:
                ...
                if model!=None:
                    for para in model.parameters():
                        for num in para.data:
                            inter=torch.norm(num)
                            n+=torch.pow(inter,2)
                printer1("layer{} number: {}".format(count,n))
                count+=1'''
        ...
        if args.edge_id==1:
            ...
            msg02=["1_to_2",des_model]
            send_msg(device_sock_all_1[0],msg02)
        ...
        if args.edge_id==2:
            ...
            for i in range(9,11):
                models.append(des_model[i-9])
            msg03=recv_msg(sock_node_1,"1_to_2")
            rec_model_60=msg03[1]
            rec_model_60=scale_model(rec_model_60,-1)
            final_model=add_model(des_model,rec_model_60)
            n=0
            count=0
            for model in final_model:
                n=0
...
for i in range(200):
    if args.edge_id==0: 
        ...
        local_train(i)
        test(models, testloader, "testcd", i)