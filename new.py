...
sock_node.connect(('localhost', 52040))
...
def local_train():
    ...
    left_model=[]
    for j in range(8,11):
        left_model.append(models[j])
    models=[]   
    for j in range(8):
        models.append(rec_model[j])
    ...
    if args.edge_id==1:
        ...
        #send_msg(device_sock_all_2[2],msg08)
        msg13=recv_msg(device_sock_all_2[0],"3_to_2")
        rec_model_7=msg13[1]
        msg24=recv_msg(device_sock_all_2[1],"4_to_3")
        rec_model_12=msg24[1]
        #msg25=recv_msg(device_sock_all_2[2],"5_to_3")
        #ec_model_13=msg25[1]
        des_model=copy.deepcopy(add_model(rec_model_1,left_model))
        des_model=copy.deepcopy(add_model(des_model,rec_model_7))
        des_model=copy.deepcopy(add_model(des_model,rec_model_12))
        #des_model=copy.deepcopy(add_model(des_model,rec_model_13))
    ...
    if args.edge_id==2:
        ...
        des_model=copy.deepcopy(add_model(rec_model_2,left_model))
        des_model=copy.deepcopy(add_model(des_model,rec_model_5))
        des_model=copy.deepcopy(scale_model(des_model,0.33333))
        for j in range(8,11):
            models.append(des_model[j-8])
    ...
    if args.edge_id==3:
        ...
        des_model=copy.deepcopy(add_model(rec_model_8,left_model))
        des_model=copy.deepcopy(scale_model(des_model,0.5))
        for j in range(8,11):
            models.append(des_model[j-8])
    ...
    '''if args.edge_id==4:
        msg26=recv_msg(sock_node_5,"2_to_3")
        rec_model_15=msg26[1]
        msg27=["5_to_3",left_model]
        send_msg(sock_node_5,msg27)
        des_model=copy.deepcopy(add_model(rec_model_15,left_model))
        des_model=copy.deepcopy(scale_model(des_model,0.5))
        for j in range(8,11):
            models[j]=copy.deepcopy(des_model[j-8])
'''
    '''for para in models[10].parameters():
        print(args.edge_id)
        print("after")
