...
if args.edge_id==2:
    ...
    sock_node_3.connect(('localhost', 54481))'''
...
trainset = datasets.ImageFolder('/data/pcqu/Dataset/Dataset/cifar10/device_5_noniid_rate_05/device'+str(args.edge_id)+'/', transform=transform)
...
for i in range(200):
    local_train()
    test(models, testloader, "testp2p5", i)
    for i in range(len(models)):
        models[i]=models[i].to(device_gpu)