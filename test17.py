...
listening_sock.bind(('localhost', 54400+args.edge_id))
...
if args.edge_id==1:
    ...
    sock_node_1.connect(('localhost', 54400))
...
if args.edge_id==2:
    ...
    sock_node_2.connect(('localhost', 54400))
    sock_node_3=socket.socket()
    sock_node_3.connect(('localhost', 54401))

if args.edge_id==3:
    sock_node_4=socket.socket()
    sock_node_4.connect(('localhost', 54401))

if args.edge_id==4:
    sock_node_5=socket.socket()
    sock_node_5.connect(('localhost', 54401))
...
trainset = datasets.ImageFolder('/data/pcqu/Dataset/Dataset/cifar10/device_5_noniid_rate_07/device'+str(args.edge_id)+'/', transform=transform)
...
def local_train():
    ...
    if args.edge_id==4:
        ...
        des_model=add_model(rec_model_15,models)
        des_model=scale_model(des_model,0.5)
        models=des_model
...
for i in range(200):
    local_train()
    test(models, testloader, "testp2p0.7", i)
    for i in range(len(models)):
        models[i]=models[i].to(device_gpu)